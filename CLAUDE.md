...
- **Vercel deploy model (docs 2026-08, decided 2026-09-05):** one Python function. `vercel.json` sets
  `framework: fastapi`, `buildCommand: cd web && npm ci && npm run build`, `functions["api/main.py"].maxDuration = 60`
  (the parser ran 14.6 s live; Hobby default is 10 s), and the cron. `pyproject.toml` `[tool.vercel] entrypoint =
  "api.main:app"` plus `[tool.vercel.fastapi.static] cdn = true`. The SPA is registered in `api/main.py` with
  `app.frontend("/", directory="web/dist", fallback="index.html")`, which Vercel promotes to the CDN at build time.
